  ...
  output$skew = renderUI(
{
  ...
  output$sampling.dist = renderPlot({
    ...
    if (error)
      ...
      ndens=density(ndist)
      nhist=hist(ndist, plot=FALSE, breaks = input$nbreaks)
      
      if (input$dist == "rnorm"){
        hist(ndist, breaks = input$nbreaks,
        ...
        hist(ndist, breaks = input$nbreaks,
                  main=paste("Distribution of means of ", k, 
                               " random samples, each\nconsisting of ", n, 
                               " observations from a ", distname, sep=""), 
      ...
      lx = max(abs(pop - m_samp))
      xarr = seq(m_samp-lx, m_samp+lx, length.out = 1000)
      lines(xarr, dnorm(xarr, mean=m_samp, sd=sd_samp), col=COL[2], lwd=3)
      box()
    }
  })

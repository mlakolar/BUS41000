...
{ 
  ...
  output$b = renderUI(
  {
    ...
    if (input$tail %in% c("middle","both"))
    {
      ...
      {
          value = round(qf(.96,as.numeric(input$df1),as.numeric(input$df2)),digits=2)
          min   = 0
          max   = round(qf(.995,as.numeric(input$df1),as.numeric(input$df2))*1.05,digits=2)
          step  = 0.01
      }
      ...
      {
          value = round(qchisq(.96,as.numeric(input$df)),digits=2)
          min   = 0
          max   = round(qchisq(.995,as.numeric(input$df)),digits=2)
          step  = 0.01
      }
  ...
  { 
    ...
    if (input$dist == "rchisq") {
        ...
        chiTail(L=L,U=U,M=M, df=input$df, xlim = c(0,round(qchisq(.995,input$df),digits=2)+1))
    ...
    if (input$dist == "rf") {        
        ...
        FTail(L=L,U=U,M=M,df_n=input$df1, df_d=input$df2, xlim = c(0,round(qf(.995,input$df1,input$df2),digits=2)+1))
  ...
  output$plotCDF = renderPlot({      
      ...
      if(input$dist == "rnorm") {
          ...
          normTailCDF(m=input$mu, s=input$sd, L=L, U=U, M=M, axes=3, cex.axis=1.5)
          title(main="CDF: Normal Distribution")
      }
      ...
      if (input$dist == "rt") {
          ...
          normTailCDF(m=0, s=1, df=input$df, L=L, U=U, M=M, axes=3, cex.axis=1.5)
          title(main="t Distribution")
      }
      ...
      if (input$dist == "rchisq") {
          ...
          chiTailCDF(L=L,U=U,M=M, df=input$df, xlim = c(0,round(qchisq(.995,input$df),digits=2)+1))
          title(main="CDF: Chi^2 Distribution")
      }
      ...
      if (input$dist == "rf") {        
          ...
          FTailCDF(L=L,U=U,M=M,df_n=input$df1, df_d=input$df2, xlim = c(0,round(qf(.995,input$df1,input$df2),digits=2)+1))
          title(main="CDF: F Distribution")
      }

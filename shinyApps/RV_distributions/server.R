...
shinyServer(function(input, output, session){
	...
	output$distName <- renderUI({
		if(input$disttype=="Discrete"){
			...
			radioButtons("dist","Distribution:",selected="norm",
	...
	getPDF <- reactive({
	    if (input$disttype=="Discrete") return(data.frame(x=double(), y=double()))
	    
		dist <- switch(input$dist,
			beta=dbeta2, cauchy=dcauchy2, chisq=dchisq2, exp=dexp2, F=df2, gam=dgamma2, lap=dlaplace2, # continuous
			logi=dlogis2, lognorm=dlnorm, norm=dnorm, pareto=dpareto2, t=dt2, unif=dunif, weib=dweibull2
			)
		...
		xmax = max( dat()[[1]] )
		xrange = xmax - xmin
		xseq = seq(from=xmin - 0.1*xrange, to=xmax + 0.1*xrange, length.out = 5000)
		argList <- list(x=xseq)
		for(i in 1:len) argList[[names(f)[i]]] <- def.args[i]
		return(data.frame(x=xseq, y=do.call(dist,argList)))
	})
	...
	output$NBIN <- renderUI({
	    if(input$disttype=="Continuous") numericInput("nbins","Number of bins",10)
	})
	output$plotPDF <- renderUI({
		if(input$disttype=="Continuous") checkboxInput("density_pdf","Plot PDF", FALSE)
	})
	...
	doPlot <- function(margins){
		if(length(input$dist)){
			...
			if(input$disttype=="Continuous"){
			    ...
			    xmax = max( d )
			    xrange = xmax - xmin
				out = hist(d,main=expr,xlab="Observations",ylab="Density",col="orange",cex.main=1.5,cex.axis=1.3,cex.lab=1.3,prob=T,
				     breaks=input$nbins, xlim=c(xmin - 0.1*xrange,xmax + 0.1*xrange))
				hist(d,main=expr,xlab="Observations",ylab="Density",col="orange",cex.main=1.5,cex.axis=1.3,cex.lab=1.3,prob=T,
				         breaks=input$nbins,
				         xlim=c(xmin - 0.1*xrange,xmax + 0.1*xrange),
				         ylim=c(0, max(pdfData[,"y"], out$density)))

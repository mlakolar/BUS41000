n.default <- 500
x.default <- seq(from=0, to=1, length.out = 1000)
...
rpareto2 <- function(n=n.default,pareto.location=1,pareto.shape=3){ rpareto(n,scale=pareto.location,shape=pareto.shape) }
...
# continuous r* wrapper functions
dbeta2 <- function(x=x.default,beta.shape1=2,beta.shape2=2){ dbeta(x,shape1=beta.shape1,shape2=beta.shape2) }
dcauchy2 <- function(x=x.default,cau.location=0,cau.scale=1){ dcauchy(x,location=cau.location,scale=cau.scale) }
dchisq2 <- function(x=x.default,chisq.df=1){ dchisq(x,df=chisq.df) }
dexp2 <- function(x=x.default,exp.rate=1){ dexp(x,rate=exp.rate) }
df2 <- function(x=x.default,F.df1=1,F.df2=15){ df(x,df1=F.df1,df2=F.df2) }
dgamma2 <- function(x=x.default,gam.shape=1,gam.rate=1){ dgamma(x,shape=gam.shape,rate=gam.rate) }
dlaplace2 <- function(x=x.default,lap.location=0,lap.scale=1){ dlaplace(x,location=lap.location,scale=lap.scale) }
dlogis2 <- function(x=x.default,logi.location=0,logi.scale=1){ dlogis(x,location=logi.location,scale=logi.scale) }
dpareto2 <- function(x=x.default,pareto.location=1,pareto.shape=3){ dpareto(x,scale=pareto.location,shape=pareto.shape) }
dweibull2 <- function(x=x.default,weib.shape=1,weib.scale=1){ dweibull(x,shape=weib.shape,scale=weib.scale) }
dt2 <- function(x=x.default,t.df=15){ dt(x,df=t.df) }
